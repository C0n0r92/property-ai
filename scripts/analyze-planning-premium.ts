 ...
 *
 * Methodology:
 * 1. Fetch all planning applications from ArcGIS (granted, last 5 years)
 * 2. Match to sold properties by address similarity
 * 3. For each matched property, find comparables (same postcode, type, beds, sold ±6 months)
 * 4. Compare price/sqm of permitted vs non-permitted properties
 *
 * Run: npx ts-node scripts/analyze-planning-premium.ts
 */
...
interface MatchedProperty {
  ...
  matchScore: number;
  workType: string;
  monthsBetweenGrantAndSale: number;
}
...
function detectWorkType(description: string): string {
...
function extractStreetName(address: string): string | null {
  // Remove house number and get street name
  const withoutNumber = address.replace(/^\d+[a-z]?\s*/i, '');
  const parts = withoutNumber.split(/[,]/);
  return parts[0]?.trim().toLowerCase() || null;
}

function calculateAddressMatchScore(planningAddr: string, propertyAddr: string): number {
...
async function fetchPlanningData(): Promise<PlanningApplication[]> {
  ...
  // Fetch granted applications from last 5 years
  ...
  while (true) {
    ...
    params.set('resultRecordCount', batchSize.toString());

    const response = await fetch(`${baseUrl}?${params}`);
    const data = await response.json() as {
      features?: Array<{ attributes: PlanningApplication }>;
    };
...
function matchPlanningToProperties(
  planning: PlanningApplication[],
  properties: Property[]
): MatchedProperty[] {
  ...
  // Filter to residential-looking applications (exclude large developments)
  const residentialPlanning = planning.filter(p => {
    const units = p.NumResidentialUnits || 0;
    return units <= 2; // Single houses/apartments only
  });
  ...
  for (const app of residentialPlanning) {
    ...
    // Skip if work type is 'other' (too vague)
    if (workType === 'other') continue;

    // Find matching property sales AFTER grant date
    for (const prop of properties) {
      if (!prop.scrapedAt || !prop.areaSqm || prop.areaSqm <= 0) continue;

      const saleDate = new Date(prop.scrapedAt);

      // Must be sold AFTER permission granted
      if (saleDate <= grantDate) continue;

      // Within reasonable timeframe (6 months to 3 years after grant)
...
function findComparables(
  ...
  return allProperties.filter(p => {
    // Exclude self and other matched properties
...
function analyzeByWorkType(
  ...
  // Group by work type
  const byWorkType = new Map<string, MatchedProperty[]>();
  ...
  for (const [workType, typeMatches] of byWorkType) {
    ...
    const withPermissionPrices: number[] = [];
    const withPermissionDetails: Array<{
      address: string;
      pricePerSqm: number;
      postcode: string;
      beds: number;
    ...
    // Find comparables without permission
    const withoutPermissionPrices: number[] = [];
    ...
    results.push({
      ...
      withPermission: {
        ...
        properties: withPermissionDetails.slice(0, 10), // Top 10 examples
      },
      withoutPermission: {
...
function analyzeByPostcode(
  ...
  // Group by postcode
  const byPostcode = new Map<string, MatchedProperty[]>();
  for (const match of matches) {
